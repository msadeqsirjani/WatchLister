...
{
    public VideoListResultModel(List<T> items,
        long totalItems,
        string pageToken,
    ...
    public static VideoListResultModel<T> Create(List<T> items,
        long totalItems,
        string pageToken,
        string nextPageToken,
        string previousPageToken,
    ...
    public VideoListResultModel<TU> Map<TU>(Func<T, TU> map) where TU : notnull =>
        VideoListResultModel<TU>.Create(Items.Select(map).ToList(), TotalItems, PageToken, NextPageToken,
            PreviousPageToken, PageSize);
}